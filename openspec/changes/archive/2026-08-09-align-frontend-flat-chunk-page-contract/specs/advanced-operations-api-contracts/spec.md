...
- **AND** request filters SHALL type `kind` as `PARENT | CHILD | FLAT` rather than an arbitrary string

#### Scenario: Preserve virtual flat response kinds
- **WHEN** a bounded page requested with `kind=FLAT` returns unparented chunks
- **THEN** the frontend SHALL accept records whose persisted `kind` is `CHILD` and whose `parentChunkId` is null
- **AND** SHALL not require or invent a response `kind=FLAT` value
...
The system SHALL expose reusable raw API functions and TanStack Query hooks for the documented chunking, chunk-read, reprocessing, and advanced-search routes, and SHALL serialize request bodies and query parameters exactly as documented. Bounded chunk-page filters SHALL accept only the typed request selectors `PARENT`, `CHILD`, and virtual `FLAT` at the frontend call boundary. The virtual selector depends on backend commit `2c4527b` being deployed.
...
- **AND** a flat population request SHALL serialize the exact query parameter `kind=FLAT`
...
- **AND** a `kind=FLAT` page key SHALL differ from the corresponding `kind=CHILD` page key
