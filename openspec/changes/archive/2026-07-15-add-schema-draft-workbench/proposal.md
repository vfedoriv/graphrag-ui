...
- Add durable analysis start, status polling, paged run history and per-source outcomes, partial/failed result handling, reload recovery, and retry controls.
...
- Uses the typed `DraftGuidance` value returned by draft create, list, detail, metadata update, and guidance update responses for safe round-trip editing.
- Uses `DraftResponse.currentAnalysis` plus paged `GET .../analysis-runs` history to recover active/recent work after navigation or reload; run summaries expose currentness, retryability, lineage, and status locations.
- Uses the typed candidate page contract, keeping analyzer `recommendationState` separate from persistent `effectiveReviewState` and `latestDecisionId`.
- Adopts the standard zero-based `page`, `size`, `totalElements`, and `content` envelope for candidates and nested analysis `sourceOutcomes`; the removed legacy list-plus-count fields are not supported.
