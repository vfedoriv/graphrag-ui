...
describe('schemaDraftReleaseApi', () => {
  ...
  it('creates and retries paged plans with explicit scope and resnapshot flags', async () => {
    ...
    await schemaDraftReleaseApi.retryPlan('kb-1', 'plan-1', { mode: 'RESNAPSHOT_UNRESOLVED' })
    ...
    expect(fetchMock.mock.calls[1][1]?.body).toBe(JSON.stringify({ mode: 'RESNAPSHOT_UNRESOLVED' }))
