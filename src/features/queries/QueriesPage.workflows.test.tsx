...
describe('queries workflows', () => {
  ...
  it('runs ask, generate, validate, and execute flows and keeps exactly four endpoint tabs', async () => {
      ...
      if (url.endsWith('/queries/execute')) {
        return jsonResponse(200, {
          ...
          rows: [{
            ...
            tags: ['graph', { level: 3 }],
            node: { _type: 'node', elementId: 'person-1', labels: ['Person', 'Engineer'], properties: { name: 'Ada', tenure: 7 } },
          }],
          rowCount: 1,
          executionTimeMs: 1,
        })
    ...
    expect(screen.getByTestId('queries-endpoint-tabs-tab-ask-query')).toBeInTheDocument()
    expect(screen.getByTestId('queries-endpoint-tabs-tab-generate-cypher')).toBeInTheDocument()
    expect(screen.getByTestId('queries-endpoint-tabs-tab-validate-cypher')).toBeInTheDocument()
    expect(screen.getByTestId('queries-endpoint-tabs-tab-execute-cypher')).toBeInTheDocument()
    ...
    await waitFor(() => {
      ...
      expect(urls.some((u) => u.includes('/queries/hybrid-search'))).toBe(false)
    })
