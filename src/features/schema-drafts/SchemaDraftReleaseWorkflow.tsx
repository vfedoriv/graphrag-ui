...
import { isEvaluationTerminal, isPlanTerminal, type EvaluationIneligibilityReason, type EvaluationMetrics } from './schemaDraftReleaseTypes'
...
export function SchemaDraftReleaseWorkflow({ draft }: { draft: DraftResponse }) {
  ...
  useEffect(() => {
    ...
    void queryClient.invalidateQueries({ queryKey: queryKeys.reprocessingPlanHistory(draft.knowledgeBaseId, draft.id, planHistoryPage, 10, 'SCHEMA_ACTIVATION') })
  ...
  return <div className='stack-lg'>
    ...
    <Stage title='4. Explicit reprocessing plan' description='Create a bounded plan only while this published schema is active. Scope and retry snapshot behavior are explicit.'>
      ...
      {plan.data ? <><div className='grid three'><Metric label='Plan' value={`${plan.data.status} · ${plan.data.id}`} /><Metric label='Progress' value={`${plan.data.succeededDocuments}/${plan.data.totalDocuments} succeeded`} /><Metric label='Unresolved' value={`${plan.data.failedDocuments} failed · ${plan.data.staleDocuments} stale · ${plan.data.blockedDocuments} blocked`} /></div>{plan.data.items.content.map((item) => item.status === 'BLOCKED' ? <Alert key={item.id} tone='info' title={`Blocked: ${item.documentId}`} message='The target schema is no longer active; this is a safety stop, not a processing failure.' /> : item.status === 'BLOCKED_TARGET_CHANGED' ? <Alert key={item.id} tone='info' title={`Target changed: ${item.documentId}`} message='The snapshotted chunk, profile, embedding, or schema target changed before processing.' /> : item.status === 'STALE_SOURCE' ? <Alert key={item.id} tone='info' title={`Changed document: ${item.documentId}`} message='The document changed after the plan snapshot and was not processed under the old content.' /> : null)}<Table headers={['Document', 'Status', 'Failure', 'Prior item', 'Completed']} rows={plan.data.items.content.map((item) => [item.documentId, item.status, item.failureCategory ?? '—', item.priorItemId ?? 'Original item', formatDate(item.completedAt)])} rowKeys={plan.data.items.content.map((item) => item.id)} /><Pager page={planItemPage} size={plan.data.items.size} total={plan.data.items.totalElements} onPage={setPlanItemPage} />{isPlanTerminal(plan.data.status) && (selectedPlanSummary?.retryable ?? plan.data.retryable) ? <><p className='text-xs'>Retry resnapshots unresolved documents using the current chunking and processing target. Prior successes remain historical.</p><Button onClick={() => { if (window.confirm('Retry unresolved plan work with a fresh snapshot?')) release.retryPlan.mutate({ knowledgeBaseId: draft.knowledgeBaseId, planId: plan.data.id, payload: { mode: 'RESNAPSHOT_UNRESOLVED' } }, { onSuccess: (result) => setPlanId(result.planId) }) }}>Retry unresolved work</Button></> : null}</> : null}
