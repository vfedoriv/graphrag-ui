...
import { formatCapturedConcurrency, formatDurationMillis, formatFailureCode } from './analysisPresentation'
...
function Sources({ draft, readOnly }: { draft: DraftResponse; readOnly: boolean }) {
  ...
  return <div className='stack-lg'>
    {!readOnly ? <div className='grid three'>
      ...
      <form className='stack' onSubmit={(event) => { event.preventDefault(); if (file) workflow.addFile.mutate({ knowledgeBaseId: draft.knowledgeBaseId, draftId: draft.id, revision: draft.revision, file }, { onSuccess: () => setFile(null) }) }}><h3>Discovery evidence file</h3><Input aria-label='Discovery evidence file' type='file' onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><small>This file influences schema discovery, stays private to this draft, and does not enter Documents. It cannot be used as a held-out evaluation document.</small><Button type='submit' disabled={!file} isPending={workflow.addFile.isPending}>Upload discovery evidence</Button></form>
    </div> : null}
...
function Analysis({ draft, readOnly }: { draft: DraftResponse; readOnly: boolean }) {
  ...
  const activeAnalysis = Boolean(
    (draft.currentAnalysis && !isTerminalAnalysisStatus(draft.currentAnalysis.status))
    || history.data?.content.some((item) => !isTerminalAnalysisStatus(item.status))
    || (run.data && !isTerminalAnalysisStatus(run.data.status)),
  )
  ...
  return <div className='stack-lg'>
    <div className='button-row'>{!readOnly ? <Button variant='primary' disabled={activeAnalysis} isPending={workflow.startAnalysis.isPending} onClick={() => workflow.startAnalysis.mutate({ knowledgeBaseId: draft.knowledgeBaseId, draftId: draft.id, revision: draft.revision }, { onSuccess: (result) => setRunId((result as { runId: string }).runId) })}>Start analysis</Button> : null}{run.data?.canRetry && isTerminalAnalysisStatus(run.data.status) && !readOnly ? <Button isPending={workflow.retryAnalysis.isPending} onClick={() => workflow.retryAnalysis.mutate({ knowledgeBaseId: draft.knowledgeBaseId, draftId: draft.id, runId: run.data.id, revision: draft.revision }, { onSuccess: (result) => setRunId((result as { runId: string }).runId) })}>Retry analysis</Button> : null}{activeAnalysis ? <StatusBadge label='Analysis active' tone='warning' /> : null}</div>
    ...
    {run.data ? <div className='stack'><div className='grid three'><div className='metric'><strong>{run.data.status}</strong><small>Run {run.data.id} · {run.data.currentResult ? 'current' : 'stale'}</small></div><div className='metric'><strong>{run.data.succeededSources}/{run.data.totalSources} succeeded</strong><small>{run.data.failedSources} failed</small></div><div className='metric'><strong>{run.data.aggregateRevisionId ?? 'No aggregate'}</strong><small>{run.data.failureCategory ?? 'No failure category'} · {run.data.retryable ? 'retryable' : 'not retryable'}</small></div></div>
      <div className='stack'><h3>Captured execution policy</h3><p className='text-xs'>These execution limits were captured for this run.</p><div className='grid three'><div className='metric'><strong>{formatCapturedConcurrency(run.data.effectiveSourceConcurrency)}</strong><small>Source concurrency</small></div><div className='metric'><strong>{formatDurationMillis(run.data.effectiveSourceTimeoutMillis)}</strong><small>Source timeout</small></div><div className='metric'><strong>{formatDurationMillis(run.data.effectiveRequestTimeoutMillis)}</strong><small>Request timeout</small></div></div></div>
      {run.data.sourceOutcomes.content.length ? <Table headers={['Source', 'Status', 'Revision', 'Chunks', 'Execution', 'Failure']} rows={run.data.sourceOutcomes.content.map((outcome) => [outcome.sourceId, outcome.status, outcome.sourceRevision, outcome.chunkCount, outcome.reused ? 'Reused' : 'Executed', outcome.failureCategory ? <span>{outcome.failureCategory}<small className='block'>{formatFailureCode(outcome.failureCode)} · {outcome.retryable ? 'Retryable failure' : 'Non-retryable failure'}</small></span> : '—'])} rowKeys={run.data.sourceOutcomes.content.map((outcome) => outcome.id)} /> : null}<Pager page={outcomePage} size={run.data.sourceOutcomes.size} total={run.data.sourceOutcomes.totalElements} onPage={setOutcomePage} /></div> : <EmptyState title='No selected analysis run' description='Start analysis or choose a historical run below.' />}
    <div className='stack'><h3>Recent analysis history</h3>{history.data?.content.length ? <Table headers={['Run', 'Status', 'Currentness', 'Progress', 'Execution', 'Retry', 'Aggregate', 'Lineage', 'Created']} rows={history.data.content.map((item) => [<button className='link-button' type='button' onClick={() => setRunId(item.id)}>{item.id}</button>, item.status, item.current ? 'Current' : 'Stale', `${item.succeededSources}/${item.totalSources} succeeded`, <span>{formatCapturedConcurrency(item.effectiveSourceConcurrency)}<small className='block'>{formatDurationMillis(item.effectiveSourceTimeoutMillis)} source · {formatDurationMillis(item.effectiveRequestTimeoutMillis)} request</small></span>, <span>{item.retryable ? 'Retryable failure' : 'Non-retryable failure'}<small className='block'>{item.canRetry ? 'Retry available' : 'Retry unavailable'}</small></span>, item.aggregateRevisionId ?? '—', item.retryOfRunId ? `Retry of ${item.retryOfRunId}` : 'Original', formatDate(item.createdAt)])} rowKeys={history.data.content.map((item) => item.id)} /> : <p>No history returned.</p>}<Pager page={historyPage} size={history.data?.size ?? 10} total={history.data?.totalElements ?? 0} onPage={setHistoryPage} /></div>
  </div>
}
